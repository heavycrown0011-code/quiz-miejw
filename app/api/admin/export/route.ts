...
export async function GET(req: NextRequest) {
  ...
  const header = ['Nome','Telefone','Idade','Célula/Igreja','Quiz','Pontuação','Máximo','Acertos','Total de perguntas','Pedido de oração','Deseja acompanhamento','Autorização','Data']
  const lines = [
    header.map(csv).join(','),
    ...rows.map(r => [r.participant_name,r.phone,r.age,r.cell_name || r.leader_name,r.quiz_title,r.score,r.max_score,r.correct_answers,r.total_scored_questions,r.prayer_request,r.wants_follow_up ? 'Sim' : 'Não',r.consent_given ? 'Sim' : 'Não',r.submitted_at].map(csv).join(',')),
  ]
